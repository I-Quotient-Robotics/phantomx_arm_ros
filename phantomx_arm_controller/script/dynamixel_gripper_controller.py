...
class DynamixelGripper:
  ...
  def __init__(self):
    ...
    try:
      self.name = rospy.get_param('~name', default = 'dynamixel_gripper_controller')
      ...
      self.joint_name = rospy.get_param('~joint_name', default = 'finger_1_joint')
      self.prismatic_command = rospy.get_param('~prismatic_command', default = '/finger_1_joint/command')

      self.open_gripper_angle = rospy.get_param('~open_gripper_angle', default = 0.0)
      self.close_gripper_angle = rospy.get_param('~close_gripper_angle', default = -3.14)

      self.open_gripper_distance = rospy.get_param('~open_gripper_distance', default = 0.015)
...
def main():
  ...
  dynamixel_gripper = DynamixelGripper()
  
  dynamixel_gripper.start()
